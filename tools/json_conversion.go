...
func DecodeJsonBinary(data []byte, useDecimal bool) ([]byte, error) {
	d := jsonBinaryDecoder{useDecimal: useDecimal}
...
func (d *jsonBinaryDecoder) decodeObjectOrArray(data []byte, isSmall bool, isObject bool) interface{} {
	...
	if isObject {
		...
		for i := 0; i < count; i++ {
			...
			keys[i] = UnsafeGetString(data[keyOffset : keyOffset+keyLength])
		}
	}
...
func (d *jsonBinaryDecoder) decodeOpaque(data []byte) interface{} {
	...
	case MYSQL_TYPE_NEWDECIMAL:
		return d.decodeDecimal(data, d.useDecimal)
	case MYSQL_TYPE_TIME:
		return d.decodeTime(data)
	case MYSQL_TYPE_DATE, MYSQL_TYPE_DATETIME, MYSQL_TYPE_TIMESTAMP:
		return d.decodeDateTime(data)
	default:
		return UnsafeGetString(data)
	}

	return nil
}

func (d *jsonBinaryDecoder) decodeDecimal(data []byte, useDecimal bool) interface{} {
	precision := int(data[0])
	scale := int(data[1])

	v, _, err := ParseDecimalType(data[2:], precision, scale, useDecimal)
	d.err = err
